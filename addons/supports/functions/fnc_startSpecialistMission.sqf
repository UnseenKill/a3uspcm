...
/* ----------------------------------------------------------------------------
Function: A3USPCM_supports_fnc_startSpecialistMission

Description:
    Callback function to start a specialist mission.

Parameters:
    0: _supportType - Support type <STRING>
    1: _supportClass - Class of support solider <STRING>
...
TRACE_1(QFUNC(startSpecialistMission),_this);

params["_markerX","_supportType"];

INFO_3("%1(): Rescue specialist task initialization started, marker: %2, support: %3.",QFUNC(startSpecialistMission),_markerX,_supportType);

private _side = [Invaders, Occupants] select ((sidesX getVariable [_markerX,sideUnknown]) == Occupants);
...
INFO_3("%1(): Side: %2, difficulty: %3.",QFUNC(startSpecialistMission),str _side,str _difficulty);
...
INFO_2("%1(): city size: %2.",QFUNC(startSpecialistMission),str _size);
...
if (isNil "_buildings" || {count _buildings < 0}) exitWith {
    ERROR_1("%1(): Problems with city buildings (1), rerequesting new rescue mission.",QFUNC(startSpecialistMission));
    [QGVAR(eventSupportSpecialistMissionNotStarted), [_supportType]] call CBA_fnc_serverEvent;
};

private _capableBuildings = _buildings select {!(isObjectHidden _x) && (([_x] call BIS_fnc_buildingPositions) isNotEqualTo [])};

if (isNil "_capableBuildings" || {count _capableBuildings < 0}) exitWith {
    ERROR_1("%1(): Problems with city buildings (2), rerequesting new rescue mission.",QFUNC(startSpecialistMission));
    [QGVAR(eventSupportSpecialistMissionNotStarted), [_supportType]] call CBA_fnc_serverEvent;
};
...
INFO_1("%1(): Informer has spawned, waiting until players will be close to the city",QFUNC(startSpecialistMission));
...
INFO_2("%1(): Roadblocks count: %2.",QFUNC(startSpecialistMission),str _roadblockCount);
...
private _bonus = [1, 2] select _difficulty;

switch(true) do {
    case (dateToNumber date > _dateLimitNum): {
        INFO_1("%1(): Time is out, fail.",QFUNC(startSpecialistMission));
...
INFO_1("%1(): Rescue Rebel Informer clean up complete.",QFUNC(startSpecialistMission));
